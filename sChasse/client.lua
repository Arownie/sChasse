...
Citizen.CreateThread(function()
    print("Script Create By Sly Zapesti#9737.")
end)
...
RegisterNetEvent("sChasse:createBlip")
AddEventHandler("sChasse:createBlip", function(type, x, y, z)
...
RegisterNetEvent("sChasse:createBlip2")
AddEventHandler("sChasse:createBlip2", function(type, x, y, z)
	local blip = AddBlipForCoord(x, y, z)
	SetBlipSprite(blip, type)
	SetBlipScale(blip, 0.8)
	SetBlipAsShortRange(blip, true)
...
---------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
		TriggerEvent('sChasse:createBlip', 141, -1492.2, 4971.5, 64.2)
		TriggerEvent('sChasse:createBlip2', 141, 960.196105957031,-2105.53198242188,31.9526977539063)
    ...
    while true do
	   ...
	   local playerPed = GetPlayerPed(-1)
		if PlayerData.job ~= nil and PlayerData.job.name ~= 'boucher' then
       ...
       playerPed = GetPlayerPed(-1)
		...
		if pos then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 960.196105957031,-2105.53198242188,31.9526977539063, true) <= 5 then
					TriggerServerEvent('sChasse:startVente')
					Citizen.Wait(4000)
				end
				if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1492.2, 4971.5, 63.2, true) <= 8 ) then
					DrawMarker(1,-1492.2, 4971.5, 62.2, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 0, 155, 255, 200, 0, 0, 0, 0)
					drawTxt("Appuie sur 'E' pour commencer a chasser ou pour ARRETER",0,1,0.5,0.8,0.6,255,255,255,255)
						...
						if (missionRunning == true and entitySpawned == true) then	
							...
							if (GetDistanceBetweenCoords( GetEntityCoords(GetPlayerPed(-1)),entityCoords[proie].x , entityCoords[proie].y , entityCoords[proie].z, true ) < 1 and entityAlive == false) then
								if IsPedInAnyVehicle(GetPlayerPed(-1), true) == false then
									...
									RemoveBlip(entityBlip[proie])
									Citizen.InvokeNative(0xAE3CBE5BF394C9C9, Citizen.PointerValueIntInitialized(entity[proie]))
									TriggerServerEvent('sChasse:startRecup')
